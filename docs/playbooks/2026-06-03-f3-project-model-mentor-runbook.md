...
tracked = subprocess.check_output(["git", "diff", "HEAD", "--name-only"], text=True).splitlines()
untracked = subprocess.check_output(["git", "ls-files", "--others", "--exclude-standard"], text=True).splitlines()
paths = sorted(set(tracked + untracked))
print("changed_or_untracked_files:", len(paths))
for path in paths:
    print(path)

diff_parts = []
if tracked:
    diff_parts.append(subprocess.check_output(["git", "diff", "HEAD", "--", *tracked], text=True, errors="replace"))
for path in untracked:
    candidate = Path(path)
    if not candidate.is_file() or candidate.stat().st_size > 1_000_000:
        print(f"untracked_scan_skipped: {path}")
        continue
