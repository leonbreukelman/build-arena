# Maintainer CLI implementation review packet

## Verification so far

- Focused tests: `uv run pytest tests/test_maintainer_cli.py tests/test_maintainer_task_packet.py tests/test_maintainer_policy.py tests/test_openshell_policy_render.py -q` -> `.......................... [100%]`

- Ruff/Pyright: `uv run ruff check . && uv run pyright` -> `All checks passed!` and `0 errors, 0 warnings, 0 informations`
...
def _write_bundle_files(out_dir: Path, packet: TaskPacket, result: PolicyResult) -> list[Path]:
    ...
    for name, content in rendered.items():
        ...
        written.append(path)
    (out_dir / "runner-command.sh").chmod(0o644)
    return written
