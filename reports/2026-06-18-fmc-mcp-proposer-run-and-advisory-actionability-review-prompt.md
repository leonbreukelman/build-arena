# Review request: fmc-mcp proposer/advisory actionability report

You are reviewing a final owner-facing Build Arena report. Do not use tools; all evidence needed is embedded in the report. Return JSON only.

Review criteria:

- Does the report faithfully describe the bounded proposer run result as fail-closed, not success?
- Does it correctly distinguish current docs proposer failure from advisory architecture/verification actionability gaps?
- Does it accurately reflect the Opus code inspection findings included in the report?
- Does it use external examples appropriately, without overclaiming they directly solve Build Arena?
- Does the recommended next slice follow from the evidence?
- Identify any patch-before-final issues.
...
The failure also exposed the same class of problem we are discussing: the current proposal layer can make a candidate, but the gates/prompt/target semantics are still too shallow or mismatched.
...
uv run python -m arena.proposal_candidate_runner \
  --worktree <repo>/.arena/worktrees/fmc-mcp-proposer-docs-rank1-20260618T013019Z \
  --proposal-plan <repo>/.arena/runs/fmc-mcp-proposer-docs-rank1-20260618T013019Z/proposal-plan.json \
  --candidate-rank 1 \
  --provider xai \
  --api-key-env XAI_API_KEY \
  --model grok-4.3 \
  --output <repo>/.arena/runs/fmc-mcp-proposer-docs-rank1-20260618T013019Z/proposal-result.json
```
...
- The diff was reversed.
- There is no patch artifact because the run failed before patch recording.
...
- Raw: `<repo>/reports/2026-06-18-fmc-mcp-advisory-proposer-opus-inspection.json`
- Normalized: `<repo>/reports/2026-06-18-fmc-mcp-advisory-proposer-opus-inspection.normalized.json`
...
Current proposer is ready to attempt docs candidates, but the live run shows the nested-doc link contract is broken and must be fixed.
