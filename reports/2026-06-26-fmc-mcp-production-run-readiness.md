...
uv run python -m arena.repo_goal_loop \
  ...
  --allow-promotion \
  --no-dry-run \
  2> >(tee "$RUN_ROOT/stderr.txt" >&2) | tee "$RUN_ROOT/stdout.txt"
```

## What this command means

- No mock path: `--decompose-mode live`, `--apply-mode live_diff`, `--allow-live`, and explicit `--live-model grok-4.3`.
- Token spend authorization is bounded by `--live-max-calls 8`; the current planned code path uses 6 calls and leaves 2 calls of operator slack. This flag is a call ceiling, not a dollar cap.
- Planned live-call math from current code: `max_cycles * (1 live decomposition + 1 live diff + 1 repair retry)` = `2 * 3 = 6`.
- `--live-max-tokens 12000` is per live provider call. Planned requested output-token ceiling is therefore `6 * 12000 = 72000` output tokens; the authorized call ceiling would be `8 * 12000 = 96000` requested output tokens if slack were consumed. Provider input/reasoning tokens are separately metered by xAI and recorded in provider metadata when returned.
...
Live flag load-bearing code references checked in current source:

- `arena/repo_goal_loop.py:214-242` rejects live modes without `--allow-live`, explicit model, and explicit `--live-max-calls`; it also refuses inert live flags when neither live mode is selected.
- `arena/repo_goal_loop.py:319-334` passes `decompose_mode`, live provider, model, API-key env, and max tokens into `build_project_model_snapshot`.
- `arena/project_decomposer_ai.py:99-106` routes `llm_mode="live"` to `LiveProjectModelLLM`, not fixture/recorded/noop output.
- `arena/repo_goal_loop.py:561-565` routes `apply_mode="live_diff"` to `_live_diff_apply`.
- `arena/repo_goal_loop.py:580-586` constructs `OpenAICompatibleDiffTransport` with the configured provider, model, API-key env, and token cap.
- `arena/runners/diff_proposer.py:84-89` constructs an OpenAI-compatible chat client for live diff transport and requires served-model match.
- `arena/llm_adapter.py:176-180` performs the live `POST /chat/completions` request.
...
```text
usage: python -m arena.repo_goal_loop [-h] --project PROJECT --goal GOAL
                                      --artifacts-root ARTIFACTS_ROOT
                                      [--profile PROFILE]
                                      [--max-cycles MAX_CYCLES]
                                      [--allow-promotion] [--no-dry-run]
                                      [--test-command TEST_COMMAND]
                                      [--decompose-mode {fixture,recorded,off,live}]
                                      [--decompose-model-output DECOMPOSE_MODEL_OUTPUT]
                                      [--apply-mode {deterministic,live_diff}]
                                      [--allow-live]
                                      [--live-provider LIVE_PROVIDER]
                                      [--live-model LIVE_MODEL]
                                      [--live-base-url LIVE_BASE_URL]
                                      [--live-api-key-env LIVE_API_KEY_ENV]
                                      [--live-max-tokens LIVE_MAX_TOKENS]
                                      [--live-max-calls LIVE_MAX_CALLS]
...
live_max_calls=8 planned=6 slack=2
```
