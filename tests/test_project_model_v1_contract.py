...
def test_project_model_v1_schema_validates_ai_decomposer_primary_artifact(tmp_path: Path) -> None:
    ...
    assert any(error.validator == "required" and "iterationReadiness" in str(error.message) for error in legacy_errors)


def test_project_model_v1_example_validates_and_requires_iteration_readiness() -> None:
    schema = _load_json(SCHEMA_PATH)
    example = _load_json(ROOT / "docs" / "examples" / "project-model-v1-tiny.json")
    validator = Draft202012Validator(schema)

    assert sorted(validator.iter_errors(example), key=lambda error: list(error.path)) == []

    missing_iteration = dict(example)
    missing_iteration.pop("iterationReadiness")
    errors = sorted(validator.iter_errors(missing_iteration), key=lambda error: list(error.path))
    assert errors
    assert any(error.validator == "required" and "iterationReadiness" in str(error.message) for error in errors)


def test_project_model_v1_schema_validates_off_mode_noop_snapshot(tmp_path: Path) -> None:
    project = tmp_path / "repo"
    _write_tiny_repo(project)
    result = build_project_model_snapshot(
        project,
        tmp_path / "artifacts",
        project_id="tiny-v1-off",
        goal="decompose tiny repo",
        non_goals=["no buckets"],
        llm_mode="off",
    )
    ...
    assert errors == []
    assert "iterationReadiness" in v1
    assert v1["iterationReadiness"]["summary"]
...
def test_project_model_v1_schema_rejects_non_v1_shape() -> None:
    schema = _load_json(SCHEMA_PATH)
    legacy_shape = {
        "schemaVersion": "legacy-project-model",
        "id": "legacy",
        "components": [],
        "observableChecks": [],
    }
