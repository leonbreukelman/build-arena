...
A deterministic intake → proposal slice is implemented downstream of the Project Model. It is advisory and does not authorize autonomous mutation. Today it is documentation-biased by construction (see epic #25 and child issues #26–#31, which generalize it to all improvement domains and close the loop):

- `arena/project_intake_scorecard.py` — weighted project-intake scorecard CLI. Reads a Project Model snapshot, emits ranked evidence-backed findings with the explainable priority formula and profile weights. Advisory only.
- `arena/proposal_planner.py` — turns the scorecard into a deterministic `proposal-plan/v0` artifact (`docs/schemas/proposal-plan-v0.schema.json`): ranked single-file candidates with grounded intent, repo-facts block, success criterion, verification commands, and skipped-finding accounting.
...
- `arena/proposal_candidate_runner.py` — selects a ranked candidate, drives `arena/runners/diff_proposer.py`, applies the diff in a worktree, and runs the candidate's verification gate. Live transport requires an explicit `--model`; otherwise it fails closed or uses `--fake-diff-file` for tests.
- `arena/markdown_links.py` — deterministic local Markdown link + source-reference validation gate for documentation proposals (`--require-source-references` for compliance-sensitive repos).

Known limitation (tracked in epic #25): scorecard absence findings only emit hardcoded documentation targets, so the proposal component can currently only produce documentation changes. Treat any "improved the project" claim from this slice as documentation-scope until the multi-domain proposal work lands.
